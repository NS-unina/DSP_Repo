...
if($_POST['sender_selected_list_check']=='check'){
	  ...
	  $subject    =  $_POST['subject'];
...

        {
           ...
           // echo $_SESSION['camp_id'];exit;
            ...
            $update_data  = array('to_list' => $list_name,'campaign_name' => $camp_name, 'subject' => $subject ,  'support' => $camp_name, 'from_name'=>$sender_name,'from_email'=>$sender_email,'create_date' => date("Y-m-d H:i:s"));
           ...
           // echo $update_data;exit;
            ...
            $lastid = array('camp_id'=>$_SESSION['add']['camp_id']);
...

        {
            ...
            $insert_data  = array('to_list' => $list_name, 'campaign_name' => $camp_name,  'subject' => $subject, 'from_name'=>$sender_name,'from_email'=>$sender_email,'create_date' => date("Y-m-d H:i:s"),'support'=>$camp_name);
...

    {
       ...
       //echo $_SESSION['camp_id']; exit;
...

        {
            ...
            $campaign_id = array('camp_id'=>$_SESSION['add']['camp_id']);
...
/*Third step start*/
...

    {
...

        {
            ...
            $campbody_id = array('camp_id'=>$_SESSION['add']['camp_id']);
...
/*Fourth step start*/
...

    {
...

        {
            ...
            $timestamp_id = array('camp_id'=>$_SESSION['add']['camp_id']);
               ...
               echo $error;
...

                {
                    ...
                    $rows_affected = $wpdb->update( $masta_campaign, $update_data, $timestamp_id);
                     ...
                     echo "campid_".$_SESSION['add']['camp_id'];
...

    {
      ...
      // echo "1";die;
...

        {
            ...
            $camp_id = array('camp_id'=> $_SESSION['add']['camp_id']);
    ...
    if(!empty($_POST['change_status']) &&  !empty($_POST['camp_id'])){
	   ...
	   $camp_id = array('camp_id'=> $camp_id);



		$update_data  = array('status'=>$status, 'create_date'=> date("Y-m-d H:i:s"));
